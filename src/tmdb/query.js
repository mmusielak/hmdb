...
function flush() {
  node.fs.writeFileSync("cache/tmdb/cache.json", JSON.stringify(_cache, null, 2));
...
function process(item) {
  ...
  var nhash = `${item.meta.title} // ${item.meta.directors} // ${item.meta.release}`;

  if (_cache[hash]) {
    var data = _cache[hash];
    _cache[nhash] = data;
    delete _cache[hash];
    return read(item, hash, data);
  }
  else if (_cache[nhash]) {
    return true;
  }
  else {
